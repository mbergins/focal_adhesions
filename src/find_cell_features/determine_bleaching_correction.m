...
expression_levels = zeros(length(single_image_folders),1);
sum_expression_levels = zeros(length(single_image_folders),1);
for i=1:length(single_image_folders)
    image = imread(fullfile(image_dir,single_image_folders(i).name,filenames.focal_image));
