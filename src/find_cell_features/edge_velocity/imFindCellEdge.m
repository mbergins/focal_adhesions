...
%                 cell_isolated:  1 if cell is isolated 0 touches img
%                                 border
%                           
...
if CLUSTER
    ...
    % here the image is clustered in a set of modes
    % CELL_MODE defines which modes belong to the cell and
    % which belong to the background. The zero mode represents
    % the lowest pixel values, thus part of the background
    img_thresh = seg_img <= CELL_MODE;
...
elseif MANUAL_LEVEL >= 0
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%  Fixed intensity value segmentation  %%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    max_value = max(img_org(:));
    if max_value <= 1;
        MANUAL_LEVEL = MANUAL_LEVEL / BIT_DEPTH;
    end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%  Global histogram based segmentation  %%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if 0
...
if ans == -1
    ...
    if ~isempty(MU0)
        ...
        [ans, img_edge, img_bg, edge_pixel, length_edge, frame_pos, rem_pix, P0, MU0]=...
            ...
            'k_max', K_MAX, 'p0', P0, 'mu0', MU0, 'cell_mode', CELL_MODE);
    else
        ...
        [ans, img_edge, img_bg, edge_pixel, length_edge, frame_pos, rem_pix, P0, MU0]=...
            ...
            'erode_dilate',ERODE_DILATE, 'median_f',MEDIAN_F,...
            'cluster', CLUSTER, 'cluster_method', CLUSTER_METHOD, 'k_cluster', K_CLUSTER, 'k_min', K_MIN,...
            'k_max', K_MAX, 'cell_mode', CELL_MODE);                                                                            
    end  
	...
	labels_stat      = regionprops(img_labels,'Area','PixelList','Centroid');
	img_area         = [labels_stat.Area];
    object_positions = [labels_stat.Centroid];
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
    [img_area_rev_objects, j_objects] = sort(img_area);
    %get the largest object
    if 1
        % get the largest object
        img_area_rev = img_area_rev_objects(end);
        j = j_objects(end);
        object_position_old = object_positions(:,j);
    else
        % get the second largest object
        img_area_rev = img_area_rev_objects(end-1);
        j = j_objects(end-1);       
    end
    
    % calculate distances
    %dist = old_object_position-object_positions;
    %min_dist_index = min(sqrt(dist(:,1).^2+dist(:,2).^2));
	...
	if nr_found_obj<1
        ...
        % return the cluster positions so they can be used in the next time step
        varargout{1} = P0;   
        varargout{2} = MU0;
        return;
    end
    ...
    % sort the pixels into a sequence of consecutive elements
