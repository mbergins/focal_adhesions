function [max_x_residuals] = regression_bootstrap(x_data, y_data);
%
global n;
global f_hat;
global x_data;
...
%figure
%waterfall(x_lag, 1:n,corr_var1_var2);
...
plot(xx, y_hat, 'r');
title('bootstrap correlation coefficients compared with spline fit to data from cells');
hold off;
...
% plot standart error pluss spline fit to the data
figure
hold on;
...
% title('Correlation coefficient for each celland  spline fit together with STD Err and 95% CIs from bootstraping')
