plot_server_load_day <- function(data.set,file.name) {
    ...
    par(bty='n',mar=c(2.8,2.5,0,0), mgp=c(1.6,0.5,0),xpd=T,lwd=3)
    plot(data.set,typ='l',ylim=c(0,4),ylab='Server Load Level',axes=F,xlab='Time (hours)')
    axis(2);
...
plot_server_load_week <- function(data.set,file.name) {
    png(file.name,width=900,height=450,pointsize=24);
    par(bty='n',mar=c(2.8,2.5,0,0), mgp=c(1.6,0.5,0),xpd=T,lwd=3)
    plot(data.set,typ='l',ylim=c(0,4),ylab='Server Load Level',axes=F,xlab='Time (Days)')
    axis(2);
